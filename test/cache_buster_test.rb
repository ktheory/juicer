...
class CacheBusterTest < Test::Unit::TestCase
  ...
  context "cleaning files with cache busters" do
    should "remove cache buster query parameters" do
      assert_equal @filename, Juicer::CacheBuster.clean("#@filename?cb=1234567890")
    end

    should "remove cache buster query parameters, but preserve other query parameters" do
      assert_equal "#@filename?some=param&others=again", Juicer::CacheBuster.clean("#@filename?some=param&cb=1234567890&others=again")
    end
    ...
    should "remove cache buster query parameters with custom name" do
      assert_equal @filename, Juicer::CacheBuster.clean("#@filename?jcb=1234567890", :jcb)
    end

    should "remove hard cache buster" do
      assert_equal @filename, Juicer::CacheBuster.clean(@filename.sub(/(\.txt)/, '-cb1234567890\1'))
    end

    should "remove hard cache buster and preserve query params" do
      assert_equal "#@filename?hey=there", Juicer::CacheBuster.clean("#@filename?hey=there".sub(/(\.txt)/, '-cb1234567890\1'))
    end
  end

  context "creating soft cache busters" do
    should "clean file before adding new cache buster" do
      cache_buster = "cb=1234567890"
    ...
    should "include mtime as query parameter" do
      ...
      assert_equal "#@filename?cb=#{mtime}", Juicer::CacheBuster.soft(@filename)
    end
  ...
  context "hard cache busters" do
    ...
    should "include mtime in filename" do
      ...
      assert_equal "#@filename-cb#{mtime}.txt", Juicer::CacheBuster.hard("#@filename.txt")
    end
